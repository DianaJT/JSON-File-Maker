...
function createJSON() {
  ...
  fs.appendFile('awesome-nodejs.json', '{\n  github:', (err) => {
    console.error(err);
  });
      ...
      ontext(text) {
        if (onPackageLine === true) { // handling package text
          if (firstPackageDone === true) {
            fs.appendFile('awesome-nodejs.json', ',', (err) => {
              console.error(err);
            });
          }
        ...
        if (inPackage === true) {
          ...
          if (awaitingName === false) {
            ...
            fs.appendFile('awesome-nodejs.json', '      url: "'.concat(currentURL, '",\n'), (err) => {
              console.error(err);
            });
            fs.appendFile('awesome-nodejs.json', '      description: "'.concat(text, '"\n    }'), (err) => {
              console.error(err);
            });
  ...
  parser.write(fs.readFileSync('awesome-nodejs.html').toString());
}
